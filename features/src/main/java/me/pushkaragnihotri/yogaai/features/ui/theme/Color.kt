package me.pushkaragnihotri.yogaai.features.ui.theme

import androidx.compose.ui.graphics.Color

// ── Primary: Deep Violet / Indigo ──────────────────────────────────────────
val VioletLight        = Color(0xFF6750A4)
val VioletDark         = Color(0xFFD0BCFF)
val OnViolet           = Color(0xFFFFFFFF)
val VioletContainer    = Color(0xFFEADDFF)
val OnVioletContainer  = Color(0xFF21005D)

// ── Secondary: Warm Amber / Saffron ────────────────────────────────────────
val AmberLight         = Color(0xFFB5620A)
val AmberDark          = Color(0xFFFFB77C)
val OnAmber            = Color(0xFFFFFFFF)
val AmberContainer     = Color(0xFFFFDCC2)
val OnAmberContainer   = Color(0xFF3A1A00)

// ── Tertiary: Lush Teal ────────────────────────────────────────────────────
val TealLight          = Color(0xFF006A60)
val TealDark           = Color(0xFF70EFDE)
val OnTeal             = Color(0xFFFFFFFF)
val TealContainer      = Color(0xFF9EF2E4)
val OnTealContainer    = Color(0xFF00201C)

// ── Backgrounds & Surfaces (Light) ────────────────────────────────────────
val OffWhiteBg         = Color(0xFFFFFBFE)
val SurfaceLight       = Color(0xFFFFFBFE)
val SurfaceVariantLight= Color(0xFFE7E0EC)
val OnLightBg          = Color(0xFF1C1B1F)

// ── Backgrounds & Surfaces (Dark) ─────────────────────────────────────────
val DarkCharcoalBg     = Color(0xFF1C1B1F)
val SurfaceDark        = Color(0xFF1C1B1F)
val SurfaceVariantDark = Color(0xFF49454F)
val OnDarkBg           = Color(0xFFE6E1E5)

// ── Yoga Detector (camera overlay) ────────────────────────────────────────
val Gunmetal           = Color(0xFF1A1A2E)
val BrightGreen        = Color(0xFF00E676)
val AccentTeal         = Color(0xFF70EFDE)
val RedButton          = Color(0xFFB3261E)
val DarkOverlay        = Color(0x99000000)
val ConfidenceBg       = Color(0xFF004D40)  // Deep teal for confidence badge

// ── Wellness metric colours ────────────────────────────────────────────────
val WellnessSleep      = Color(0xFF7986CB)  // Indigo-ish
val WellnessStreak     = Color(0xFFFFB300)  // Amber
val WellnessSteps      = Color(0xFF42A5F5)  // Blue
val WellnessHR         = Color(0xFFE91E63)  // Pink
val WellnessCalories   = Color(0xFFEF5350)  // Red
val CardBackgroundLight= Color(0xFFFFFFFF)

// ── Status ─────────────────────────────────────────────────────────────────
val StatusGreen        = Color(0xFF4CAF50)
val StatusOrange       = Color(0xFFFF9800)
val StatusRed          = Color(0xFFF44336)

// ── Risk ───────────────────────────────────────────────────────────────────
val RiskLowLight       = Color(0xFFE8F5E9)
val RiskLowTextLight   = Color(0xFF2E7D32)
val RiskMediumLight    = Color(0xFFFFF3E0)
val RiskMediumTextLight= Color(0xFFE65100)
val RiskLowDark        = Color(0xFF1B5E20)
val RiskLowTextDark    = Color(0xFFA5D6A7)
val RiskMediumDark     = Color(0xFFE65100)
val RiskMediumTextDark = Color(0xFFFFCCBC)
